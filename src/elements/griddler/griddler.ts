...
import { DomCommsHelper } from '../dom-comms-helper';
...
export class Griddler extends CustomElementBase {
  ...
  private static readonly SIZE_MIN = 5;
  private static readonly SIZE_MAX = 50;
  private static readonly SIZE_INTERVAL = 1;
  private static readonly DEF_SIZE = 20;
  ...
  constructor() {
    ...
    // this._gridCanvas.addEventListener('mousemove', DomCommsHelper.Throttle((event: MouseEvent) => {
   ...
   */
  redraw(): void {
    const labels_w = this._cols * 5;
    const labels_h = this._rows * 5;
    const grid_w = this._cols * this._size + .5;
    const grid_h = this._rows * this._size + .5;
    ...
    for (let c = 0; c <= this._cols; c++) {
      gridContext.moveTo(c * this._size + .5, 0);
      gridContext.lineTo(c * this._size + .5, grid_h);
    }
    for (let r = 0; r <= this._rows; r++) {
      gridContext.moveTo(0, r * this._size + .5);
      gridContext.lineTo(grid_w, r * this._size + .5);
    }
    ...
    for (let ci = 0; ci <= this._cols; ci += Griddler.XY_INTERVAL) {
      gridContext.moveTo(ci * this._size + .5, 0);
      gridContext.lineTo(ci * this._size + .5, grid_h);
    }
    for (let ri = 0; ri <= this._rows; ri += Griddler.XY_INTERVAL) {
      gridContext.moveTo(0, ri * this._size + .5);
      gridContext.lineTo(grid_w, ri * this._size + .5);
    }
  ...
  private getCoords(offsetX: number, offsetY: number): { x: number, y: number } {
    return {
      x: Griddler.Round(offsetX / this._size, 0, 1, 0, this._cols),
      y: Griddler.Round(offsetY / this._size, 0, 1, 0, this._rows),
    };
  }
  ...
  private onMouseMove(event: MouseEvent) {
    ...
    if (coords.x !== this._cols) {
      this._hiliteContext.fillRect(coords.x * this._size + 1.5, 0, this._size, this.totalHeight);
    }
    if (coords.y !== this._rows) {
      this._hiliteContext.fillRect(0, coords.y * this._size + 1.5, this.totalWidth, this._size);
    }
  }
